...
if (apartment !== null)
{
    let listingDetails = document.querySelector("#listing-details");
    listingDetails.innerHTML = "";

    let listingDetailsContainer = document.createElement("div");
    listingDetailsContainer.classList.add ("listing_Details_Container");

    listingDetailsContainer.innerHTML = 
    `<img src="${apartment.picture_url}" alt="apartmentImage">` +
    `<h2>${apartment.name}</h2>` +
    `<p><b>Rating:</b> <br> ${apartment.review_scores_rating}</p>` +
    `<p><b>Price per night:</b> ${apartment.price}</p>` +
    `<p><b>Bedrooms:</b> <br> ${apartment.bedrooms}</p>` +
    `<p><b>Beds:</b> <br> ${apartment.beds}</p>` +
    `<p><b>Batrooms:</b> <br> ${apartment.batrooms_text}</p>` +
    `<p><b>Guests:</b> <br> ${apartment.accommodates}</p>` +
    `<p><b>Neighborhood:</b> <br> ${apartment.neighbourhood}</p>`;
    
   listingDetails.appendChild(listingDetailsContainer);
}
else{
